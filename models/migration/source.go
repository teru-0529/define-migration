...
func NewSourceSet(path string, github Github, useLocal bool) (*SourceSet, error) {
	...
	for i, source := range sources.SourceArray {
		...
		if !slices.Contains([]SourceType{GITHUB, LOCAL_FILE}, source.SourceType) {
			fmt.Printf(
				"Warning: 'sourceType'[\"%s\"] of 'schema'[\"%s\"] is converted \"local\" , because of 'sourceType' must be one of the [\"github\",\"local\"]\n",
				source.SourceType,
				source.SchemaName,
			)
			sources.SourceArray[i].SourceType = LOCAL_FILE
		...
		if source.SourceType == GITHUB && source.Repository == "" {
			fmt.Printf(
				"Warning: 'sourceType' of 'schema'[\"%s\"] is converted \"local\" , because of required 'githubRepository' if 'sourceType' is [\"github\"]\n",
				source.SchemaName,
			)
			sources.SourceArray[i].SourceType = LOCAL_FILE
		}
